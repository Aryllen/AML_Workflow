...
for (data.file in filenames.excel) {
  ...
  if (length(which(str_detect(sheets, coll("Fitted Parameters Static", ignore_case = TRUE)))) > 0) {
    ...
    errors <- rbind(errors, data.frame(error.sheet, data.file, NA, NA, NA))
  ...
  if (length(which(str_detect(colnames(drug.data), coll("compound", ignore_case = TRUE)))) > 0) {
    ...
    # Add filename and error code to errors table
    errors <- rbind(errors, data.frame(error.compound, data.file, NA, NA, NA))
    # Set clean flag to false
    clean <- FALSE
  }
  ...
  if (length(which(str_detect(colnames(drug.data), coll("EC50", ignore_case = TRUE)))) > 0) {
    ...
    # Add filename and error code to errors table
    errors <- rbind(errors, data.frame(error.EC50, data.file, NA, NA, NA))
    # Set clean flag to false
    clean <- FALSE
  }
  ...
  if (sum(is.na(suppressWarnings(as.numeric(drug.data[[col.EC50]])))) == dim(drug.data)[1]) {
    ...
    # Add filename and error code to errors table
    errors <- rbind(errors, data.frame(error.static, data.file, colnames(drug.data)[col.EC50], NA, NA))
    # Set clean flag to false
    clean <- FALSE
  ...
  if (length(which(str_detect(colnames(drug.data), coll("File", ignore_case = TRUE)))) > 0) {
    ...
    if (is.na(filename.patient) || filename.patient != patient) {
      ...
      # Add filename and error code to errors table
      errors <- rbind(errors, data.frame(error.id, data.file, colnames(drug.data)[col.index], patient, filename.patient))
      # Set clean flag to false
      clean <- FALSE
    ...
    # Add filename and error code to errors table
    errors <- rbind(errors, data.frame(error.id.verify, data.file, NA, patient, NA))
    # Set clean flag to false
    clean <- FALSE
  }
...
if (file.exists(paste0(output.dir, "Compound_Synonyms.csv"))) {
  ...
  if (length(synonyms) > 0) {
    ...
    apply(synonyms[,1], 1, function (x) {compounds.all <- append(compounds.all, x)})
...
if (clean) {
  ...
  if (length(err.ind) > 0) {
    sheet.errors.message <- "No drug sensitivity was found in the following Excel files. Please ensure a sheet/tab with the data has one of the following phrases in the name: Fitted Parameters Static, FitParameters, combined, Fitted Parameters, blast param statsort, or 4Database."
  ...
  if (length(err.ind) > 0) {
    compound.errors.message <- "No column called Compound was found in the drug sensitivity data for the following files."
  ...
  if (length(err.ind) > 0) {
    EC50.errors.message <- "No column called either EC50 or IC50 was found in the drug sensitivity data for the following files."
  ...
  if (length(err.ind) > 0) {
    static.errors.message <- "The EC50 or IC50 data does not appear to be numeric in the following files. The column name checked is in parentheses."
  ...
  if (length(err.ind) > 0) {
    id.errors.message <- "The patient number extracted from the filename was not the same as the patient number extracted from inside the file. Two columns were checked for, a column with the word File in it and a column with the word Sample in it. For the following files, the patient IDs from these locations did not match. "
  ...
  if (length(err.ind) > 0) {
    id.verify.errors.message <- "There was no column with the word File or Sample in the name found in the following files. Due to this, the patient ID could not be verified."
