...
for (data.file in filenames) {
  ...
  if (length(which(str_detect(sheets, coll("missense", ignore_case = TRUE)))) > 0) {
    ...
    errors <- rbind(errors, data.frame(error.miss, data.file, NA, NA, NA))
  ...
  if (length(which(str_detect(sheets, coll("indel", ignore_case = TRUE)))) > 0) {
    ...
    # Add filename and error code to errors table
    errors <- rbind(errors, data.frame(error.ind, data.file, NA, NA, NA))
    # Set clean flag to false
    clean <- FALSE
  }
  ...
  if (length(which(str_detect(colnames(var.data), coll("Sample", ignore_case = TRUE)))) > 0) {
    ...
    if (is.na(sample.patient) || sample.patient != patient) {
      ...
      # Add filename and error code to errors table
      errors <- rbind(errors, data.frame(error.id, data.file, colnames(var.data)[col.index], patient, sample.patient))
      # Set clean flag to false
      clean <- FALSE
    ...
    # Add filename and error code to errors table
    errors <- rbind(errors, data.frame(error.id.verify, data.file, colnames(var.data)[col.index], patient, NA))
    # Set clean flag to false
    clean <- FALSE
  }
}
...
if (clean) {
  file.create(paste0(output.dir, "Clean_Var.txt"))
}
# Save a done file to use as a trigger iff there were no problems
if (clean) {
  ...
  if (length(err.ind) > 0) {
    miss.errors.message <- "No missense variant data was found in the following Excel files. Please ensure a sheet/tab with the data has the word missense in the name."
  ...
  if (length(err.ind) > 0) {
    ind.errors.message <- "No indel variant data was found in the following Excel files. Please ensure a sheet/tab with the data has the word indel in the name."
  ...
  if (length(err.ind) > 0) {
    id.verify.errors.message <- "There was no column with the word sample in the name found in the following files. Due to this, the patient ID could not be verified."
